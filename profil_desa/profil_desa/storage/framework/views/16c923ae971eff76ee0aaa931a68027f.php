    ...
    * {
    ...
    <body class="font-sans antialiased m-0 p-0 flex flex-col min-h-screen">
        ...
        <div class="absolute inset-0 w-full h-full">
            <img src="<?php echo e(asset('images/bgtes.jpg')); ?>" alt="Background" class="w-full h-full object-cover">
        </div>
                    ...
                    <div class="w-full 2xl:h-[calc((100vw-24rem)*9/24)] xl:h-[calc((100vw-22rem)*9/19)] lg:h-[calc((100vw-10rem)*9/19)] md:h-[calc((100vw-8rem)*9/16)] sm:h-auto phone:h-auto flex 2xl:flex-row xl:flex-row lg:flex-row md:flex-row sm:flex-col phone:flex-col 2xl:items-start xl:items-start lg:items-start md:items-center sm:items-center phone:items-center rounded-md mb-4">
                        <div class="2xl:w-[65%] xl:w-[65%] lg:w-[60%] md:w-[60%] sm:w-full phone:w-full h-full flex flex-col justify-start items-start p-5 2xl:pr-20 xl:pr-10 phone:p-2 lg:pr-8 gap-2 2xl:order-1 xl:order-1 lg:order-1 md:order-1 sm:order-2 phone:order-2">
                            ...
                            <div class="mt-2 font-light text-justify text-base phone:text-xs text-gray-700 reveal-r animate">
                                <?php echo nl2br(e($sambutan->sambutan ?? 'Belum ada sambutan.')); ?>

                                <?php if(!empty($sambutan->nama_kepala_desa)): ?>
                                    <br><br><br>— <?php echo e(strtoupper($sambutan->nama_kepala_desa)); ?>, KEPALA DESA BANJARSARI —
                                <?php endif; ?>
                            </div>
                        </div>
                        <div class="2xl:w-auto xl:w-auto lg:w-auto md:w-auto sm:w-[60%] phone:w-[70%] h-full flex flex-col 2xl:p-8 xl:p-8 lg:p-8 md:p-7 sm:p-8 phone:p-5 sm:order-1 phone:order-1 self-center relative">
                            ...
                            <?php else: ?>
                                <img src="<?php echo e(asset('images/logo.png')); ?>" alt="Default"
                                    class="h-full w-full bg-cover bg-center self-center z-10 reveal rounded-lg shadow-product animate" />
                            <?php endif; ?>
                        </div>
                    </div>
    ...
    <div class="w-[85%] mx-auto shadow-md rounded-2xl bg-white">
        <div class="flex flex-col md:flex-row gap-6 h-[600px] p-4">
            ...
            <?php if($kontens->count()): ?>
                ...
                <?php $utama = $kontens->first(); ?>
                <div class="w-[30%] bg-yellow-600 text-white p-6 rounded-lg flex flex-col justify-between h-full shadow-lg">
                    <div>
                        <img src="<?php echo e(asset('storage/' . $utama->cover)); ?>"
                            class="rounded-md mb-4 h-64 w-full object-cover shadow" 
                            alt="Cover Konten">
                        <h3 class="text-xl font-bold leading-snug break-words whitespace-normal"><?php echo e($utama->judul); ?></h3>
                        <p class="text-sm mt-3 text-white/90 line-clamp-3">
                            <?php echo e(\Illuminate\Support\Str::limit(strip_tags($utama->deskripsi), 120)); ?>

                        </p>
                    </div>
                    <div class="mt-4">
                        <a href="<?php echo e(route('user.berita.show', $utama->id)); ?>"
    ...
    <section class="py-10 px-4">
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            <?php $__currentLoopData = $galeris; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <div x-show="filter === 'all' || filter === '<?php echo e($item->tipe); ?>'" 
                     class="relative group overflow-hidden rounded-lg shadow-lg cursor-pointer bg-gray-200" 
                     @click="openLightbox('<?php echo e($item->tipe); ?>', '<?php echo e(asset('storage/galeri/' . $item->file)); ?>', '<?php echo e($item->judul); ?>')">

                    <?php if($item->tipe == 'gambar'): ?>
                        <img src="<?php echo e(asset('storage/galeri/' . $item->file)); ?>" alt="<?php echo e($item->judul); ?>" class="w-full h-48 object-cover transform transition-transform duration-300 group-hover:scale-110">
                    <?php else: ?>
                    ...
                    <div class="absolute bottom-0 left-0 w-full p-2 bg-gradient-to-t from-black to-transparent pointer-events-none">
                        <h3 class="text-white text-sm font-semibold truncate"><?php echo e($item->judul); ?></h3>
                    </div>
                </div>
            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
        </div>
        ...
        <div class="text-center mt-8">
            <a href="<?php echo e(route('user.galeri')); ?>" class="bg-blue-500 text-white px-6 py-2 rounded-full shadow-md hover:bg-blue-600 transition-all duration-300 ease-in-out transform hover:scale-105">
                Lihat Semua Galeri
            </a>
        </div>
    </section>
...
<br>
    ...
    <section class="py-10">
        ...
        <div x-ref="slider" class="flex overflow-x-auto gap-6 pb-4 pl-4 md:pl-8 lg:pl-12 horizontal-scrollbar snap-x snap-mandatory scroll-smooth" style="-webkit-overflow-scrolling: touch; scrollbar-width: none;">
            <?php $__currentLoopData = $aparatur; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $aparat): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <div :id="'aparatur-' + <?php echo e($index); ?>" class="flex-shrink-0 w-64 bg-gray-50 rounded-lg shadow-md text-center overflow-hidden transform transition-all duration-500 snap-center" 
                     :class="{'filter grayscale-0 scale-105': activeIndex === <?php echo e($index); ?>, 'filter grayscale scale-90 opacity-80': activeIndex !== <?php echo e($index); ?> }">
                    <img src="<?php echo e($aparat->foto ? Storage::url($aparat->foto) : asset('images/profile.png')); ?>"
                         alt="Foto <?php echo e($aparat->nama); ?>"
                         class="w-full h-56 object-cover object-center">
                    <div class="p-4">
                        <h3 class="text-lg font-semibold text-gray-800 truncate" title="<?php echo e($aparat->nama); ?>"><?php echo e($aparat->nama); ?></h3>
                        <p class="text-sm text-gray-500"><?php echo e($aparat->jabatan); ?></p>
                    </div>
                </div>
            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
        </div>
...
<div class="w-[85%] mx-auto shadow-md rounded-2xl p-1 bg-white">
    ...
    if (e.target === modal) {
        ...
        function carousel() {
            return {
                currentIndex: 0,
                slides: <?php echo json_encode($slides, 15, 512) ?>, // <-- pastikan ini menghasilkan data valid
                prev() {
...
<?php echo $__env->make('layouts.landing', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\Jopu\Downloads\profil_desa\profil_desa\resources\views/welcome.blade.php ENDPATH**/ ?>